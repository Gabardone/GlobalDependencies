...
/**
 ...
 The macro will declare two additional types and also insert them as a `typealias` into the attachee protocol. One is
 a key type, adopting `DependencyKey`, which is used for dependency management and overrides. One is a protocol
 of the same name as the attachee, postfixed with `Dependency` and with a single property that returns an `any
 ...
 The other one is a type adopting `DependencyKey` which will be used by `GlobalDependencies` for override management.
